...
目前SRS内置的SIP服务器只实现了简单的 `Register`、`Invite` 指令，而如果要实现GB/T-28181的全部功能，势必会引入复杂的上层业务逻辑，甚至一些个性化需求。所以，我们开发了一个独立的外置的SIP服务器。而SRS，只需开放几个简单的API接口，这样就保证了其媒体转发服务器的单一属性，同时兼顾了跟第三方SIP信令服务器对接的需求。

仓库地址位于：https://github.com/ossrs/srs-sip
