...
采集的部署实例参考：[Ingest](http://ossrs.net/srs.release/wiki/v4_CN_SampleIngest)
...
Ingest需要在编译时打开：`--with-ingest`。参考：[Build](http://ossrs.net/srs.release/wiki/v4_CN_Build)

Ingest默认使用自带的ffmpeg，也可以不编译ffmpeg，使用自己的编转码工具。禁用默认的ffmpeg在编译时指定`--without-ffmpeg`即可。参考：[Build](http://ossrs.net/srs.release/wiki/v4_CN_Build)
...
* 其他参考转码的配置：[FFMPEG](http://ossrs.net/srs.release/wiki/v4_CN_FFMPEG)
