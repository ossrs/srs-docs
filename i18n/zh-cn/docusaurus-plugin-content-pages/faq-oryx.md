...
* [Getting Started](#getting-started)：如何起步: 买了Oryx怎么用，怎么开头，怎么入门，怎么起步，怎么开始折腾。
...
使用宝塔安装Oryx时，需要输入管理后台的域名，例如`bt.yourdomain.com`，会自动创建管理后台的网站。

> Note: 使用宝塔安装时，若希望使用IP访问，可以设置为`bt.yourdomain.com`，然后在宝塔中把`srs.stack.local`这个网站设置为默认网站。

若使用其他方式安装也一样可以，只需要将你的域名解析到Oryx的IP即可。

有几种方式设置域名解析：

1. DNS域名解析，在你的域名供应商的后台，设置一个A记录，指向Oryx的IP。
```text
A bt.yourdomain.com 121.13.75.20
```
2. Linux/Unix修改本地`/etc/hosts`文件，将域名解析到Oryx的IP。
```text
121.13.75.20 bt.yourdomain.com
```
3. Windows修改本地的`C:\Windows\System32\drivers\etc`文件，将域名解析到Oryx的IP。
```text
121.13.75.20 bt.yourdomain.com
```
...
Oryx支持Docker镜像，安装脚本，腾讯云Lighthouse和CVM镜像，DigitalOcean镜像，其他平台可以用宝塔安装。

推荐使用Docker直接安装，还可以装多个，注意要用Ubuntu 20+系统：
* Docker镜像安装：[这里](../docs/v6/doc/getting-started-oryx#docker)

Oryx支持HELM安装，请参考[srs-helm](https://github.com/ossrs/srs-helm)的说明。
...
可以选择使用Docker方式启动Oryx，可以非常容易跑非常多的Oryx实例，这些实例都是隔离的，互不影响，可以将机器资源利用上。
...
> Note: 注意端口不要重复，挂载的数据目录也不要重复，保持两个Oryx的完全独立。
...
若需要推流到两个Oryx实例，则需要指定端口，比如推流到这两个Oryx：
...
当然也不是意味着你就可以启动上万个Oryx，你应该关注你的CPU和内存，以及机器的带宽是否充足。

## How to Set up Free HTTPS

Oryx支持申请免费HTTPS证书，而且可以申请多个域名的证书，还可以自动续期。比如，以下HTTPS网站的证书，都是跑了Oryx后一键自动申请的：
...
3. 在Oryx的 `系统设置 > HTTPS > 自动HTTPS证书` 填写你的域名，多个用分号分割，点申请就行了。

> Note: 申请域名就行了，不要再上传，申请了就可以了，不需要再上传一次。

> Note: 若使用宝塔安装Oryx，可以选择在宝塔中申请，或者在Oryx中申请。

若出现申请失败的错误，消息为`Could not obtain certificates: error: one or more domains had a problem`，可能原因是：

* 域名没有解析到Oryx的IP，必须使用DNS将域名解析到Oryx的IP，而不能设置hosts的方式。
* Oryx的IP必须是公网可以访问的IP，必须是互联网上任何人都能访问的IP，而不是内网可以访问的IP。
...
Oryx是在容器中运行的，所以它这个`/data`说的是容器中的路径，所以你可以在启动Oryx的容器时，把你host机器的
...
你在Oryx中就可以输入`/data/my-upload/my-file.mp4`访问这个文件。
...
若使用docker直接启动Oryx，可以将目录挂载为其他路径，比如：

```bash
docker run -v /your-host-dir:/data/record
```

若使用其他方式安装Oryx，则可以将默认数据目录软链接为其他目录，比如：

```bash
rm -rf /data && ln -sf /your-host-dir /data
```

> Note: 请不要直接软链接`/data/record`，因为Oryx是在docker中运行，看不到你软链接的目录。
...
因此，主播停止推流后，Oryx不会立刻生成录制文件，而是会等待一定时间，超时后才会生成。
...
> Remark: 特别注意，挂载存储后，需要重启Oryx才能访问到挂载的目录。

然后在Oryx的录制中，选择`设置录制规则 > 录制后处理 > 拷贝录制文件`，输入文件夹`/data/srs-s3-bucket`，这样在录制生成文件后，
...
特别注意，必须要挂载到`/data`目录的子目录下，Oryx才能正常访问到，若只能挂载到其他目录，则建议使用Docker启动Oryx，
并指定`-v /your-host-dir:/data/srs-s3-bucket`，这样Oryx就可以访问到了。

## How to Record a Specific Stream

如何按特定规则录制，如何录制指定的流。

Oryx允许您配置Glob匹配规则，该过滤器仅录制符合定义规则的流。要设置Glob匹配规则，请选择
...
这是因为刚刚安装后，Oryx启动需要时间，等待3到5分钟后刷新页面，就可以了。
...
看起来OBS的链路更短更简单，还不用经过Oryx不用付钱，为何Oryx还要做转推，OBS这个方案有什么缺点？
...
Oryx其实也是会有带宽翻倍，但是它是下行带宽翻倍，因为它做了一次转换，本质上是其他平台从Oryx这里下载流。下行/下载带宽一般都是更有保障的。而且Oryx和平台之间，都是服务器之间的BGP带宽，比家里到平台质量更有保障。

## How SRS Restreams to Custom Platforms

SRS的多平台转推，是可以推到自定义的直播平台，比如推到视频号的推流地址和流密钥，也可以填任何其他直播平台的。

> Note: Oryx之所以分成视频号和B站等平台，是为了提供更好的引导，这些平台的RTMP地址格式都是类似的，所以可以随意填，Oryx不会验证具体的平台。
...
默认情况下，Oryx将虚拟直播的输入比特率限制为5Mbps，您可以从`系统配置>限流>限流设置`更改限制以设置更高的限制。

## How to Setup the Font Style for AI Transcript

如何设置AI自动字幕的字体样式？Oryx支持设置字体样式，格式为FFmpeg的force_style，
...
> Note: Netflix使用的字体是`Consolas`，Oryx暂时不支持，这里使用`Roboto`代替。
...
如果使用Docker版本，可以更换Oryx中的FFmpeg为自定义版本，启动时指定命令：
...
宝塔海外版本叫[aaPanel](https://aapanel.com)，请使用aaPanel，安装软件很快，Oryx也会切换到海外的镜像下载。
...
SRS是Oryx的媒体引擎，详细差异请查看[与SRS比较](../docs/v6/doc/getting-started-oryx#compare-to-srs)。
