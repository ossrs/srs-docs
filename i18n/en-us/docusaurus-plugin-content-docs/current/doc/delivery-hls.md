...
* hls_ts_file: The filename of the ts file, with some variables in [dvr variables](http://ossrs.net/srs.release/wiki/v4_EN_DVR#custom-path). And, variable [seq] is the ts seqence number.
```bash
...
SRS support delivering pure audio stream by HLS. The audio codec requires AAC, user must transcode other codecs to AAC, read [Usage: Transcode2HLS](http://ossrs.net/srs.release/wiki/v4_EN_SampleTranscode2HLS)
